...
class OracleClient(BaseWeb3Client):
    ...
    def __init__(self):
        ...
        # Convert to checksum address only if not empty
        if oracle_address_raw:
            try:
                self.oracle_address = self.to_checksum_address(oracle_address_raw)
            except ValueError as e:
                logger.error(f"❌ Invalid PLATFORM_REVENUE_ORACLE_ADDRESS: {e}")
                self.oracle_address = None
        else:
            self.oracle_address = None
            logger.warning("⚠️  PLATFORM_REVENUE_ORACLE_ADDRESS not set - Oracle functionality disabled")
        
        # Validate address is not zero address
        if self.oracle_address and self.oracle_address == self.to_checksum_address("0x" + "0" * 40):
            logger.warning("⚠️  PLATFORM_REVENUE_ORACLE_ADDRESS is zero address")
            self.oracle_address = None
    ...
    async def submit_monthly_report(
        ...
        try:
            ...
            # Get current nonce
            finance_address = getattr(self.finance_account, 'address', None)
            if not finance_address:
                raise ValueError("Finance account has no address attribute")
            
            nonce = self.w3.eth.get_transaction_count(finance_address)
            ...
            transaction = self.contract.functions.submitMonthlyReport(
                ...
                total_expenses_wei,
                breakdown_tuple
            ).build_transaction(cast(TxParams, {
                'from': finance_address,
                'nonce': nonce,
                'gas': 500000,
