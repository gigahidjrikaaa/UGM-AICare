...
async def get_user_stats(db: AsyncSession) -> UserStats:
    """Calculate overall user statistics for the admin dashboard."""
    ...
    active_30d = (await db.execute(
        select(func.count(User.id)).filter(User.last_activity_date >= month_ago)
    )).scalar() or 0

    active_7d = (await db.execute(
        select(func.count(User.id)).filter(User.last_activity_date >= week_ago)
    )).scalar() or 0

    new_today = (await db.execute(
        select(func.count(User.id)).filter(func.date(User.created_at) == today)
    )).scalar() or 0

    avg_sentiment = (await db.execute(select(func.avg(User.sentiment_score)))).scalar() or 0
