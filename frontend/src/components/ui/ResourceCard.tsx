        ...
        <div className="p-4 sm:p-5 bg-gradient-to-br from-white/5 via-white/10 to-white/5 rounded-xl border border-white/10 shadow-lg hover:border-white/20 transition-all duration-300 ease-in-out flex flex-col h-full">
            ...
            {targetAudience && (
                <div className="flex items-center text-xs text-gray-400 mb-2">
                    <FiUsers className="mr-2 flex-shrink-0 text-ugm-blue-light" /> For: {targetAudience}
                </div>
            )}
            ...
            <div className="mt-auto space-y-1.5 pt-3 border-t border-white/10">
                {operatingHours && (
                    <div className="flex items-start text-sm text-gray-300">
                        <FiClock className="mr-2.5 mt-0.5 flex-shrink-0 text-gray-400" size={15}/>
                        <span>{operatingHours}</span>
                    </div>
                )}
                {address && (
                    <div className="flex items-start text-sm text-gray-300">
                        <FiMapPin className="mr-2.5 mt-0.5 flex-shrink-0 text-gray-400" size={15}/>
                        <span>{address}</span>
                    </div>
                )}
                {phone && (
                    <div className="flex items-center text-sm text-gray-300">
                        <FiPhone className="mr-2.5 flex-shrink-0 text-gray-400" size={15}/>
                        <a href={`tel:${cleanPhoneNumberForTelLink(phone)}`} className="hover:text-[#FFCA40] transition-colors">{phone}</a>
                    </div>
                )}
                {whatsapp && (
                     <div className="flex items-center text-sm text-gray-300">
                        <FiMessageSquare className="mr-2.5 flex-shrink-0 text-gray-400" size={15}/>
                ...
                {email && (
                    <div className="flex items-center text-sm text-gray-300">
                        <FiMail className="mr-2.5 flex-shrink-0 text-gray-400" size={15}/>
                        <a href={`mailto:${email}`} className="hover:text-[#FFCA40] transition-colors">{email}</a>
                    </div>
                )}
                {website && (
                    <div className="flex items-center text-sm text-gray-300">
                        <FiGlobe className="mr-2.5 flex-shrink-0 text-gray-400" size={15}/>
