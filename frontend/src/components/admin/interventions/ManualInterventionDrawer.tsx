...
interface ManualSafetyCoachingDrawerProps {
  open: boolean;
  onClose: () => void;
...
export function ManualInterventionDrawer({ open, onClose, campaigns, onSubmit, initialUser }: ManualSafetyCoachingDrawerProps) {
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-white">Manual safety coaching</h2>
          <button
            onClick={onClose}
            className="rounded-full border border-white/10 p-2 text-white/60 hover:border-white/40 hover:text-white"
            title="Close drawer"
            aria-label="Close manual safety coaching drawer"
            type="button"
          >
        ...
        <p className="mb-5 text-sm text-white/60">
          Trigger a targeted outreach with human context. Use an existing safety coaching template or provide a custom
          message.
        </p>

        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="user-id" className="block text-xs uppercase tracking-wide text-white/50">
              User ID
            </label>
            <input
              id="user-id"
          ...
          <div>
            <label htmlFor="campaign-select" className="block text-xs uppercase tracking-wide text-white/50">
              Use existing safety plan
            </label>
            <select
              id="campaign-select"
              ...
              className="mt-2 w-full rounded-lg border border-white/20 bg-transparent px-3 py-2 text-sm text-white focus:border-[#4C8BF5] focus:outline-none"
              aria-label="Use existing safety plan"
            >
              <option value="" className="bg-[#00122F]">
                -- Create ad-hoc safety plan --
              </option>
          ...
          {!formState.campaign_id && (
            <>
              <div>
                <label htmlFor="subject-title" className="block text-xs uppercase tracking-wide text-white/50">
              ...
              <div>
                <label htmlFor="safety-message" className="block text-xs uppercase tracking-wide text-white/50">
                  Message
                </label>
                <textarea
                  id="safety-message"
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              <label htmlFor="delivery-method" className="block text-xs uppercase tracking-wide text-white/50">
                Delivery method
              </label>
              <select
                id="delivery-method"
            ...
            <div>
              <label htmlFor="schedule-for" className="block text-xs uppercase tracking-wide text-white/50">
                Schedule for
              </label>
              <input
                id="schedule-for"
          ...
          <div>
            <label htmlFor="coach-notes" className="block text-xs uppercase tracking-wide text-white/50">
              Coach notes
            </label>
            <textarea
              id="coach-notes"
