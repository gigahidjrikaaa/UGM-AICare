...
const AgentsCommandCenterPage: React.FC = () => {
  ...
  // Group events by correlationId for structured display
  ...
  // Persist selection and auto-select newest if none stored
  useEffect(() => {
    if (runs.length > 0) {
  ...
  // sendCommand must be defined before retryRun; move retryRun below sendCommand

  const cancelRun = useCallback(async (runId: number) => {
    try {
      await fetch(`${API_BASE}/runs/${runId}/cancel`, { method: 'POST', credentials: 'include' });
    } catch {/* ignore */}
  }, []);
