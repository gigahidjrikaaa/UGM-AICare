...
export default function FlagsPage() {
  ...
  if (loading) return <div className="text-gray-300">{t('loading', { fallback: 'Loading…' })}</div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-white">{t('title', { fallback: 'Flagged Sessions' })}</h1>
        <div className="flex items-center gap-2">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 bg-white/10 border border-white/20 rounded-md text-white">
            <option value="" className="bg-gray-800">{t('filter.all', { fallback: 'All' })}</option>
            <option value="open" className="bg-gray-800">{t('filter.open', { fallback: 'Open' })}</option>
            <option value="reviewing" className="bg-gray-800">{t('filter.reviewing', { fallback: 'Reviewing' })}</option>
            <option value="resolved" className="bg-gray-800">{t('filter.resolved', { fallback: 'Resolved' })}</option>
          </select>
          <Button variant="outline" className="text-white" onClick={() => load()}>{t('refresh', { fallback: 'Refresh' })}</Button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-lg p-3">
        <div className="flex items-center gap-3">
          <label className="inline-flex items-center gap-2 text-sm text-gray-200">
            ...
            {t('selectAll', { fallback: 'Select all' })}
          </label>
          {anySelected && (
            <span className="text-sm text-gray-300">{selectedIds.length} {t('selected', { fallback: 'selected' })}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" className="text-white" disabled={!anySelected} onClick={bulkClose}>{t('closeSelected', { fallback: 'Close Selected' })}</Button>
          <Button variant="outline" className="text-white" disabled={!anySelected} onClick={bulkAddTag}>{t('addTagSelected', { fallback: 'Add Tag to Selected' })}</Button>
        </div>
      </div>

      <div className="space-y-4">
        {flags.length === 0 && (
          <div className="text-gray-400">{t('empty', { fallback: 'No flags found.' })}</div>
        )}
        {flags.map((f) => (
          <div key={f.id} className="bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-white font-mono text-sm">{t('session', { fallback: 'Session' })} …{f.session_id.slice(-8)}</div>
                <div className="text-xs text-gray-300">{t('flag', { fallback: 'Flag' })} #{f.id} • {new Date(f.created_at).toLocaleString()}</div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Link className="text-[#FFCA40] text-sm" href={`/admin/conversations/session/${f.session_id}`}>{t('viewSession', { fallback: 'View Session' })}</Link>
              </div>
            </div>
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-300 mb-1">{t('reason', { fallback: 'Reason' })}</label>
              ...
              <div>
                <label className="block text-xs text-gray-300 mb-1">{t('notes', { fallback: 'Notes' })}</label>
            ...
            <div className="mt-3">
              <label className="block text-xs text-gray-300 mb-1">{t('tags', { fallback: 'Tags' })}</label>
...
function TagEditor({ initial, onChange }: { initial: string[]; onChange: (tags: string[]) => void; }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <input value={value} onChange={(e) => setValue(e.target.value)} placeholder={t('addTag', { fallback: 'Add tag' })} className="px-2 py-1 bg-white/10 border border-white/20 rounded-md text-white text-xs" />
      <Button variant="outline" className="text-white text-xs" onClick={() => { const v = value.trim(); if (!v) return; if (!tags.includes(v)) { const next = [...tags, v]; setTags(next); onChange(next); } setValue(''); }}>{t('add', { fallback: 'Add' })}</Button>
    </div>
  );
}
